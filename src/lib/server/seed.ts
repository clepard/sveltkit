...
import { pages, user } from '$lib/server/db/schema';
import { createPageDocument } from '$lib/server/page-document';
...
const starterPages = [
	{
		slug: 'home', title: 'Northwind Studio', eyebrow: 'Independent creative studio',
	...
	{
		slug: 'work', title: 'Our Work · Northwind Studio', eyebrow: 'Selected work',
		headline: 'Useful ideas, made beautifully.',
	...
	{
		slug: 'about', title: 'About · Northwind Studio', eyebrow: 'About the studio',
		headline: 'Small team. Wide perspective.',
...
export async function seedSite(db: Database, auth: Auth, password: string) {
	if (await db.query.user.findFirst({ where: eq(user.id, 'admin') })) {
		throw new Error('The administrator is already provisioned');
	}
	await auth.api.signUpEmail({
		body: { name: 'Administrator', email: 'admin@local.invalid', password }
	});
	if (!await db.query.user.findFirst({ where: eq(user.id, 'admin') })) {
