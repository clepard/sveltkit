<script lang="ts">
	...
	let pendingUpload = $state<{ targetId: string; imageId: string } | null>(null);
	let showMedia = $state(false);
	let imageTarget = $state<{ id: string; label: string } | null>(null);
	let selectingImages = $state(false);
	...
	$effect(() => {
		...
		if (!pending || !data.images.some((image) => image.id === pending.imageId)) return;
		draft.images[pending.targetId] = pending.imageId;
		pendingUpload = null;
	});
	...
	function openImage(id: string, label: string) {
		...
		selectingImages = false; selectedImageIds = [];
	}
	function closeMedia() {
		showMedia = false;
	...
	function toggleImageSelection(id: string) {
		selectedImageIds = selectedImageIds.includes(id)
			? selectedImageIds.filter((selectedId) => selectedId !== id)
			: [...selectedImageIds, id];
	}
	...
	function selectUploadFile(event: Event) {
		...
		if (uploadFilename) input.form?.requestSubmit();
	}
	...
	const enhanceUpload: SubmitFunction = ({ formData, cancel }) => {
		...
		return async ({ result, update }) => {
			if (result.type === 'success' && result.data?.uploaded && typeof result.data.imageId === 'string' && imageTarget) {
				pendingUpload = { targetId: imageTarget.id, imageId: result.data.imageId };
			}
			await update({ reset: result.type === 'success', invalidateAll: result.type === 'success' });
			if (result.type === 'success') uploadFilename = '';
			uploading = false;
		};
	};
	const enhanceDeleteImages: SubmitFunction = ({ cancel }) => {
		if (!selectedImageIds.length || deletingImages) {
			cancel();
			return;
		}
		deletingImages = true;
...
{#if showMedia && imageTarget}
	<div class="media-overlay"><button class="media-backdrop" onclick={closeMedia} aria-label="Close image picker"></button>
		<div class="media-dialog" role="dialog" aria-modal="true"><header><div><p>Media library</p><h2>{selectingImages ? 'Manage images' : `Choose ${imageTarget.label}`}</h2><small>{selectingImages ? 'Select unused images to remove from your library.' : 'Select an existing image or upload a new one.'}</small></div><div class="media-header-actions"><button type="button" class:active={selectingImages} onclick={toggleImageManager}>{selectingImages ? 'Cancel selection' : 'Select images'}</button><button type="button" class="media-close" onclick={closeMedia} aria-label="Close media library">×</button></div></header>
			<div class="media-grid">
				<button type="button" class="media-item" class:selected={!selectedImage && !selectingImages} disabled={selectingImages} onclick={() => chooseImage()}><div class="mini-art"></div><span>Graphic placeholder</span></button>
				{#if uploading}
				...
				{#each data.images as image}
					<button type="button" class="media-item" class:selected={!selectingImages && selectedImage?.id === image.id} class:marked={selectedImageIds.includes(image.id)} onclick={() => selectingImages ? toggleImageSelection(image.id) : chooseImage(image.id)}>
						{#if selectingImages}<b class="select-check">{selectedImageIds.includes(image.id) ? '✓' : ''}</b>{/if}
						<img src={'/media/' + image.variants.at(-1)?.filename} alt={image.alt} /><span>{image.alt}</span>
					</button>
				{/each}
			</div>
			{#if selectingImages}
				<div class="delete-bar">
					<span>{selectedImageIds.length ? `${selectedImageIds.length} selected` : 'Select images you no longer need'}</span>
					<form method="POST" action="?/deleteImages" use:enhance={enhanceDeleteImages}>
						<input type="hidden" name="imageIds" value={JSON.stringify(selectedImageIds)} />
						<button class="delete-button" disabled={!selectedImageIds.length || deletingImages}>{deletingImages ? 'Deleting…' : `Delete${selectedImageIds.length ? ` (${selectedImageIds.length})` : ''}`}</button>
					</form>
					{#if form?.deleteError}<p class="delete-error">{form.deleteError}</p>{/if}
				</div>
			{/if}
			<div class="upload-panel">
				...
				<form method="POST" action="?/upload" enctype="multipart/form-data" use:enhance={enhanceUpload}>
					<label class="file-picker">
						<input type="file" name="image" accept="image/jpeg,image/png,image/webp,image/avif" required disabled={uploading} onchange={selectUploadFile} />
						<span>{uploading ? 'Uploading image…' : 'Choose an image'}</span><small>{uploading ? uploadFilename : 'Uploads immediately · alt text uses the filename'}</small>
					</label>
